...
from .utils import root, duration2timedelta, hash_id, MetadataException, avg_domain_distance, Observable, load_callable
from .constants import NS, config, EVENT_REPOSITORY_RELOAD, EVENT_STORE_RELOAD
...
class MDRepository(Observable):
    ...
    def __init__(self):
        ...
        self.rm.subscribe(self.reload)
        self.store_class = load_callable(config.store_class)
        self.store = None

    def reload(self, *args, **kwargs):
        if 'type' in kwargs and kwargs['type'] == EVENT_REPOSITORY_RELOAD:
            store = self.store_class()
            for r in self.rm.items():
                store.update(r.url, tid=r.name)
            self.store = store
            self.event(type=EVENT_STORE_RELOAD)
